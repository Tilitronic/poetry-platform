...
| `session_spawn` with non-terminal state, real child ID, and distinct `parent_session` | No open generation                                          | Start             | Treat the producer's first correlated child-session row as authoritative and anchor the generation to its `seq` |
