...
- D3 amendment (rev-1 Major fix, preset-NAME): a slash-less env value resolves as a preset NAME via `presets[<name>].orchestrator.model`, first array entry, read live from `.opencode/oh-my-opencode-slim.jsonc` at runtime so config edits track without a guard change. This is a single-key lookup, not a merge re-implementation, so it stays within D3's env-only intent (the env value remains the sole intent carrier). Any lookup failure (missing file, bad JSON, missing preset, unusable ref) degrades to the unparsable one-log no-op. Slash-form values keep the strict slash-split path unchanged.
