...
Per §2.1's "manual-sync contract" paragraph, the two sources are manually synced at spec-author time, and "bumping either version requires updating BOTH the Dockerfile ARGs and the `[tools]` entry." This change **enforces that contract** with a validator that reads both sources and asserts parity. The validator does NOT modify either source — it only reads.
...
- **Two-source comparison contract:** parse each source independently, normalize both sides (quote/CRLF/whitespace stripping), compare. No cross-source mutation. No "fix drift on detection" — the validator only reports.
...
- Associative arrays (`declare -A`) — the parsed pin map is a pair of shell variables per source (`MISE_NODE`, `MISE_PNPM`, `DOCKER_NODE`, `DOCKER_PNPM`), not a map.
...
This is a deliberate design choice: the validator's data model is four named pins (two per source), not a dynamic key-value map. Shell variables are sufficient; associative arrays would be over-engineering.

## Files changed

| File                                          | Change                                                                                                                                                                                                                                                                                                                                                                                                             | Notes                                                                                                            |
| --------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------- |
| `scripts/check-pin-sync.sh`                   | **New file** — bash-3 compatible, `set -euo pipefail`, standalone source-parity validator. Four functions: `parse_reference()` (reads `.mise.toml`), `parse_dockerfile()` (reads `Dockerfile.dev`), `compare()` (normalizes + compares), `aggregate()` (accumulates results + emits summary). Exit precedence `2 > 1 > 0`. ~60–80 lines.                                                                         | Exit 0 iff all pins match; exit 1 on any mismatch; exit 2 on defective source (missing file, duplicate key).     |
| `scripts/__tests__/check-pin-sync.bats`       | **New file** — 10-case FAKE-mock matrix (T1–T10). Fixture helpers promoted to `test-helper.bash`. Each test plants a fixture tree under `$BATS_TEST_TMPDIR` with controlled `.mise.toml` / `Dockerfile.dev` contents; the validator runs against the fixture.                                                                                                                                                       | bats NEVER reads the real repo `.mise.toml` / `Dockerfile.dev` (except one S4-style structural assertion).        |
| `scripts/__tests__/test-helper.bash`          | **Extended** — promote `install_fakes()`-style tree builder from `check-tools.bats` into a shared `setup_pin_sync_tree <with_mise_toml 0|1> <with_dockerfile 0|1> <mise_node_pin> <mise_pnpm_pin> <docker_node_pin> <docker_pnpm_pin>` helper. Refactor `check-tools.bats`'s inline `setup_tree()` to call the shared helper (no behavior change to existing tests).                                              | Pure refactor of shared fixture plumbing; existing `check-tools.bats` behavior is unchanged.                     |
...
- **`tools/opencode-docker/Dockerfile`** — DIA-050 explicitly scopes this change to the primary `.mise.toml` ↔ `Dockerfile.dev` pair.
...
make test-shell
   ...
   │        │
   │        ├── resolve paths: ROOT_DIR/.mise.toml, ROOT_DIR/Dockerfile.dev
   │        │       │
   │        │       └── either missing? ──▶ exit 2 (INFRA: source defective)
   ...
   │        │                                                 summary: 0 ok, 0 fail (infra)
   │        │
   │        ├── parse_dockerfile(DOCKERFILE_DEV) ──▶ (DOCKER_NODE, DOCKER_PNPM)
   │        │       │
   ...
   │        │       │       │
   │        │       │       ├── equal ──▶ ok: <tool> <version> (parity)    [stdout]
   │        │       │       │
   │        │       │       └── differ ──▶ fail: <tool> — .mise.toml=<X> Dockerfile.dev=<Y>  [stderr]
   │        │       │                      accumulate mismatch
   │        │
   │        ├── aggregate(): emit summary: N ok, M fail   [both streams]
   │        │
   ...
   └── bash scripts/__tests__/bats-wrapper.sh
            ...
            └── exec bats __tests__/
                   │
                   └── check-pin-sync.bats  (Gate A — FAKE-mock, 10 cases)
...
| ------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------- |
| **Validator script** (`scripts/check-pin-sync.sh`)     | Pure bash-3 function of two source files → ok/fail/infra + summary. Four functions: parse_reference / parse_dockerfile / compare / aggregate. | `scripts/__tests__/check-pin-sync.bats` (Gate A) + `make check-pin-sync` (Gate B)      | Gate A: FAKE-mock behavioral (10 cases, bats uses fixture trees only). Gate B: real-host.           |
...
This change **promotes these helpers to `scripts/__tests__/test-helper.bash`** for reuse across both the `check-tools.bats` and `check-pin-sync.bats` suites. The promotion is a pure refactor: existing `check-tools.bats` behavior is unchanged, but the helper definitions move from `check-tools.bats`'s inline block to `test-helper.bash`. A new `setup_pin_sync_tree` helper is added to `test-helper.bash` that extends the tree-builder pattern with both a `.mise.toml` AND a `Dockerfile.dev` fixture (with configurable pin values per source).
...
- **Gate A (FAKE-mock seam):** every test in `check-pin-sync.bats` plants fixture files under `$BATS_TEST_TMPDIR` and drives behavior via controlled content. Fast, hermetic, CI-runnable, covers the validator's branching logic (match, mismatch, missing files, duplicate keys, quote/CRLF/whitespace normalization). Does NOT verify the validator runs against the real repo files.
- **Gate B (real-host seam):** the owner runs `bash scripts/check-pin-sync.sh` (or `make check-pin-sync`) against the real `.mise.toml` and `Dockerfile.dev`. Slow, non-hermetic, only runs on the owner's machine. DOES verify the validator runs against the real repo files.

The two seams are complementary: Gate A covers logic, Gate B covers reality. The invariant — Gate A bats NEVER read the real repo `.mise.toml` / `Dockerfile.dev` (with the documented exception of one S4-style structural assertion) — is the enforcement line between them.

## Design constraints and trade-offs

### Why a standalone validator instead of extending `check-tools.sh`

- **Separation of concerns:** `check-tools.sh`'s job is to probe the mise-managed runtime (activation + version parity with `.mise.toml` pins). The new validator's job is to assert parity BETWEEN the two sources of truth (`.mise.toml` and `Dockerfile.dev`). Mixing the two conflates runtime probing with source-parity checking.
...
- **Defense in depth:** quote/CRLF/whitespace stripping in `compare()` handles the three known variants (double-quoted, single-quoted, unquoted) that could appear across the two sources. The normalization is cheap (a `sed` pipeline) and robust.
...
### Why ~60–80 line budget

- **Sufficient for the surface:** four functions (parse_reference / parse_dockerfile / compare / aggregate) + main flow + headers/comments fits comfortably in 60–80 lines. Going above 80 would indicate over-engineering (e.g., reaching for associative arrays or dynamic key lists).
- **Maintainability cap:** a validator under 80 lines is readable in a single screen; code review is cheap. A validator over 100 lines starts to invite "why is this so complicated?" questions.
