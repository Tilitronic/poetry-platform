...
T1 — check-pin-sync.sh core (parse+compare+aggregate + exit codes)
 │  creates scripts/check-pin-sync.sh (~60-80L, new, bash-3 compatible)
...
T2 — bats suite T1-T10 + test-helper.bash promotion
 │  creates scripts/__tests__/check-pin-sync.bats (~120L, new, 10 scenarios)
...
**Blockers:** none
**Vertical slice:** the validator script + minimal verification. After T1, `bash -n scripts/check-pin-sync.sh` exits 0, and `bash scripts/check-pin-sync.sh` exits 0 on the current repo (where both sources agree at 24.18.0 / 10.33.0).
...
- New file, executable, ~60–80 lines, bash-3 compatible.
...
- Resolve `MISE_TOML="${ROOT_DIR}/.mise.toml"` and `DOCKERFILE_DEV="${ROOT_DIR}/Dockerfile.dev"`.

**Sub-step (b): Implement the INFRA preflight**

- Check both files exist: `[ -f "${MISE_TOML}" ]` and `[ -f "${DOCKERFILE_DEV}" ]`. Either missing → emit `fail: source defective: <file> not found at <path>` to stderr + `summary: 0 ok, 0 fail (infra)` to stderr + exit 2.
...
**Sub-step (d): Implement `parse_dockerfile`**

- Reads `$DOCKERFILE_DEV`, extracts `ARG NODE_VERSION=...` / `ARG PNPM_VERSION=...`.
...
- Tracks per-ARG occurrence counts. If any ARG appears more than once, emit `fail: source defective: Dockerfile.dev has duplicate ARG '<NAME>'` to stderr + exit 2.
- Returns via two global shell variables: `DOCKER_NODE`, `DOCKER_PNPM`.
...
- For each tool in `{node, pnpm}`: call `compare <tool> <mise_value> <docker_value>`.
  - Match → emit `ok: <tool> <version> (parity)` to stdout; increment ok counter.
  - Mismatch → emit `fail: <tool> — .mise.toml=<X> Dockerfile.dev=<Y>` to stderr; increment fail counter; accumulate mismatch.
- After the loop: emit `summary: N ok, M fail` to BOTH stdout and stderr (same shape as `check-host-jq.sh` / `validate-skills.sh`).
...
2. `bash scripts/check-pin-sync.sh` — real-host invocation. Expected: exit 0, `ok: node 24.18.0 (parity)` + `ok: pnpm 10.33.0 (parity)` + `summary: 2 ok, 0 fail`. (On the current repo, both sources agree.)
3. Bash-3 compliance grep: `grep -nE 'declare -A|\$\{!|\[\[ |\*\*|printf -v' scripts/check-pin-sync.sh` returns no matches.
4. Visual inspection: the script is under 80 lines; the four-function shape is visible; exit precedence `2 > 1 > 0` is documented in the header.
...
3. On the current repo (where `.mise.toml` pins match `Dockerfile.dev` ARGs at 24.18.0 / 10.33.0), `bash scripts/check-pin-sync.sh` exits 0 with both ok lines + summary.
...
6. If `.mise.toml` has duplicate `node` / `pnpm` key under `[tools]`, exits 2 (INFRA) with duplicate-key fail line.
7. If `Dockerfile.dev` has duplicate `ARG NODE_VERSION=...` / `ARG PNPM_VERSION=...`, exits 2 (INFRA) with duplicate-ARG fail line.
8. On pin mismatch (one or both tools), exits 1 with mismatch fail line(s); report-ALL (not fail-fast).
9. Output contract: ok→stdout, fail→stderr, summary on both streams.
10. Script is under 80 lines; four-function shape (parse_reference / parse_dockerfile / compare / aggregate) is visible.
...
- `bash scripts/check-pin-sync.sh` exit code + output (expected: exit 0 + `ok: node 24.18.0 (parity)` + `ok: pnpm 10.33.0 (parity)` + `summary: 2 ok, 0 fail`).
- Bash-3 compliance grep result (expected: no matches).
- Line count of `scripts/check-pin-sync.sh` (expected: ≤ 80).
...
**Vertical slice:** the 10-scenario bats suite + test-helper promotion. After T2, `make test-shell` passes with the 10 new cases, the existing 7 `check-tools.bats` cases still pass (refactor check), and the shared fixture helpers are reusable.

**Routing:** AGENTS.md §2.4 → `@reviewer` (two-axis: Standards + Spec fidelity)
**Commit message template:** `test(dev-infra): 10-scenario bats suite for check-pin-sync + test-helper promotion`
...
- New helper: `setup_pin_sync_tree <with_mise_toml 0|1> <with_dockerfile 0|1> <mise_node_pin> <mise_pnpm_pin> <docker_node_pin> <docker_pnpm_pin>`.
...
- Supports the quote/CRLF/whitespace variants via additional optional parameters (e.g., `setup_pin_sync_tree 1 1 "24.18.0" "10.33.0" "24.18.0" "10.33.0" "crlf"` — the last argument selects a line-ending variant).

**Sub-step (c): Write `scripts/__tests__/check-pin-sync.bats`**

- New file, ~120 lines.
- Header comment describing the 10-case matrix (T1–T10) + the FAKE-mock invariant (bats NEVER reads real repo files, except one S4-style structural assertion).
- `load test-helper`.
- 10 `@test` blocks:
  - **T1** — `all pins match -> exit 0 + ok lines + summary`.
  - **T2** — `single pin mismatch (node) -> exit 1 + mismatch fail line + summary`.
  - **T3** — `multiple pin mismatches (report-ALL) -> exit 1 + both fail lines + summary`.
  ...
  - **T8** — `quote variations (single/double/unquoted) -> exit 0 after stripping`.
  - **T9** — `CRLF line endings -> exit 0 after stripping`.
  - **T10** — `whitespace variations (extra spaces around =) -> exit 0 after stripping`.
...
1. `make test-shell` — exit 0. Existing 7 `check-tools.bats` cases still pass (refactor check). 10 new `check-pin-sync.bats` cases pass.
2. Test count delta: baseline → baseline+10. If the delta differs, the coder explains.
...
3. `scripts/__tests__/check-pin-sync.bats` exists with 10 `@test` blocks (T1–T10).
4. Every test in `check-pin-sync.bats` (except the S4 structural assertion) uses fixture files under `$BATS_TEST_TMPDIR`; no test reads the real repo `.mise.toml` / `Dockerfile.dev` for behavioral coverage.
5. T1-T10 cover every branch: match, single-mismatch, multi-mismatch (report-ALL), missing source (×2), duplicate key (×2), quote/CRLF/whitespace normalization (×3).
6. `make test-shell` exits 0 with +10 cases; existing 7 `check-tools.bats` cases still pass.

### Testing (T2)

- **RED-GREEN discipline:** the refactor of `check-tools.bats` helpers (sub-step a) is tested first — existing 7 cases must still pass. Then the new bats file is added (sub-step c) — all 10 cases pass against the T1 production code.
...
- **Invariant enforcement:** the coder verifies by inspection that no behavioral test reads `$REPO_ROOT/.mise.toml` / `$REPO_ROOT/Dockerfile.dev`. (Automated enforcement: no trivial grep can enforce this across fixture-tree indirection; the coder's inspection is the gate.)

### Verification evidence (T2 coder handoff)

- `make test-shell` exit code (expected: 0).
- Test count delta (expected: baseline → baseline+10). If different, explanation.
- Confirmation that `scripts/__tests__/check-tools.bats` 7 cases still pass (refactor check).
- Confirmation that `scripts/__tests__/check-pin-sync.bats` 10 cases pass.
...
- Edit `docs/dev-infra-audit/tickets/DIA-050.md`:
  ...
  - `Fix` section: populate with:
    > Closed by change `openspec/changes/dev-infra-pin-sync/`. `scripts/check-pin-sync.sh` asserts `.mise.toml` ↔ `Dockerfile.dev` parity for `node`/`pnpm` under `make test-shell`. 10-scenario bats suite (T1–T10) covers match/mismatch/missing/duplicate/normalization branches. Exit precedence `2>1>0` (INFRA>mismatch>match).
  - `Re-verify` section: populate with:
    > 1. `bash scripts/check-pin-sync.sh` exits 0 with `ok: node 24.18.0 (parity)` + `ok: pnpm 10.33.0 (parity)` + `summary: 2 ok, 0 fail`.
    > 2. `make test-shell` exits 0; includes 10 new `check-pin-sync.bats` cases.
...
1. **`bash -n scripts/check-pin-sync.sh`** — syntax check. Exit 0 required.
2. **`make check-pin-sync`** — standalone invocation. Expected: exit 0 + `ok: node 24.18.0 (parity)` + `ok: pnpm 10.33.0 (parity)` + `summary: 2 ok, 0 fail`.
3. **`make test-shell`** — full bats suite. Expected: exit 0, validator runs as prereq (Gate B), then bats runs (Gate A) with 10 new cases passing. Test count increases by 10 from baseline.
...
2. `make test-shell` runs `check-pin-sync` as a prereq BEFORE any bats run; bats auto-discovery runs `check-pin-sync.bats` with 10 cases. Post-change: all pass.
...
- `bash -n scripts/check-pin-sync.sh` exit code (expected: 0).
- `make check-pin-sync` exit code + output (expected: exit 0 + ok lines + summary).
- `make test-shell` exit code + summary line (expected: exit 0, test count increases by 10 from baseline).
...
- **`tools/opencode-docker/Dockerfile` parity check** — explicitly deferred per proposal scope boundary.
- **§10 routing** — explicitly N/A per proposal scope boundary.
- **Any `.sdd/` document authoring** — gap flagged in proposal, not filled here.
- **Extra bats cases beyond the 10-case matrix (T1–T10)** — explicitly excluded per proposal Testing Decisions.
...
| `bash scripts/check-pin-sync.sh` (real)    | T1 sub-step g | Exit 0 on current repo (both sources agree at 24.18.0 / 10.33.0)                           |
| Bash-3 compliance grep                     | T1/T2/T3      | No matches for `declare -A\|\$\{!\|\[\[ \|\*\*\|printf -v`                                  |
| Line count of script                       | T1 sub-step g | ≤ 80 lines                                                                                 |
| `make test-shell`                          | T2 sub-step d | Exit 0; existing 7 `check-tools.bats` cases still pass (refactor check)                    |
| `make test-shell`                          | T3 sub-step d | Exit 0; includes 10 new `check-pin-sync.bats` cases (Gate A FAKE-mock)                     |
| `make check-pin-sync`                      | T3 sub-step d | Exit 0 on current repo (Gate B real-host standalone invocation)                            |
