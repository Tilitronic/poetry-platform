...
`49d587a` (DIA-118) which wired `make test-shell` into the pre-push script. The
...
**Why did DIA-118's addition (`make test-shell`) recurse?**
-> DIA-118 wired `make test-shell` into the pre-push gate (commit `49d587a`,
...
```
commit 49d587a (DIA-118): add `make test-shell` to verify-pre-push.sh
...
**Regression commit:** `49d587a` (DIA-118, "feat(hooks): wire test-shell/test-config
...
7. [ ] Update DIA-118 ticket status to CLOSED (or create a follow-up ticket if
      DIA-118 is already closed)
...
introduced by commit `49d587a` (DIA-118). The root cause is the absence of a
