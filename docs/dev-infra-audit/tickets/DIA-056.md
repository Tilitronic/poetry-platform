...
(opencode.jsonc L249), NEVER invokes token_* at runtime → stacking trigger
eliminated.
...
**Pending (runtime-dependent, post-restart):** restart + smoke as DIA-055.
Acceptance: 2 consecutive smoke lanes complete with clean per-part outcomes, no
stacking, @ai-auditor session reusable, and the §10 Phase-5 deny proof CONFIRMED
at config or runtime level.
