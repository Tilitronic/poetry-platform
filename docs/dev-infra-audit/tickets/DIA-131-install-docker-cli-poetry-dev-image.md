# DIA-131 - Install docker CLI + compose plugin in poetry-dev image (pre-push test-config gate)

<!-- Ticket authored 2026-08-13 during the omo-slim-changes rebase push (DIA-116
...
attempts: 0
lease_expires_at: ""
files_touched: [docs/dev-infra-audit/tickets/DIA-131-install-docker-cli-poetry-dev-image.md, docs/dev-infra-audit/tickets/README.md, Dockerfile.dev]
artifacts: []
evidence: []
...
The pre-push hook (scripts/verify-pre-push.sh, DIA-118) delegates every
...
Observed 2026-08-13 while pushing the omo-slim-changes rebase branch (DIA-116
...
Applied 2026-08-13 (uncommitted, working tree; DIA-131 implementation + 6
reviewer findings):
...
5. TLS-failure fallback procedure (documented, not a shell fallback — YAGNI
   ...
   tools/opencode-docker, DIA-121), replace this apt install with the
