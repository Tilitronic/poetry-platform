...
**Pending (runtime-dependent, post-restart):** restart OpenCode → smoke
`token_export` from denied tiers (analyzer/reviewer/architector/ai-specialist/
ai-auditor/council/resource-manager) must be DENIED; orchestrator
`token_export: allow` intact. When all pass → mark VERIFIED.
