# DIA-147 - pre-push blocked: make test-shell fails inside hook (unshare 127 + guard-flag interaction suspicion)

<!-- Docs-lane ticket filed 2026-08-12 after a push attempt was blocked by the
     husky pre-push hook following the DIA-146 commit. Two distinct suspects
...
- Suspicion A (guard-flag inheritance): DIA-146's fix exports
  ...
  0), breaking assertions. DIA-146's verification ran the suite
...
Related: DIA-146 (recursion guard fix - suspicion that inherited
VERIFY_PRE_PUSH_RUNNING=1 breaks verify-pre-push.bats tests 183-191),
DIA-71 (pre-existing env-gate push blocker).
...
<!-- UPDATE 2026-08-14 (RENUMBER, NO STATUS CHANGE): ticket renumbered DIA-123 -> DIA-147 (duplicate-ID collision resolution; local campaign ticket DIA-123-deterministic-restart-detection keeps its ID). Fix commit c46d4bf ('fix(hooks): unset recursion-guard flag in bats setup + guard test (DIA-123)') exists in git log; merge 4b3dbf7 confirmed. Status stays VERIFIED (unchanged). -->
