...
- **Same-session fixes (DIA-135):** fix-loop dispatches MUST resume the SAME coder session that wrote the code (resume by task_id/session_id per A2), never a fresh instance - fixes need the implementer's context.
