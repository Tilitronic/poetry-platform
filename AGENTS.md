...
- **DIA-217 ticket gate bypass paths (DIA-260820-jlu0):** the DIA-217 hook hard-blocks any `task()` dispatch that cannot resolve to an unambiguous governing DIA ticket ID. Two no-ticket-ID escape hatches exist, each emitting an audit row (`capability_used` / `meta_task_bypass`) plus a TUI warn: (1) a pre-minted `[CAPABILITY: CAP-...]` token (scope `ticket-creation` / `ai-infra-application` / `bootstrap`), minted by the orchestrator via the `mint_capability` tool; (2) a meta-task carve-out for ticket-creation / procedural-authorization dispatches whose text contains any of `scripts/tickets new`, `create ticket`, `procedural authorization`, `meta-task`, or the strict `[META-TASK]` marker. The meta-task carve-out removes the chicken-and-egg deadlock (a fresh ticket's ID is unknown at creation time) and returns BEFORE ticket_id resolution so a stray literal DIA id in the text is never attributed. Bootstrap / ai-infra-application stay on the capability-token path (not chicken-and-egg). Every bypass is auditable; legitimate ticket-gated work is unchanged.
