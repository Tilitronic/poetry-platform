...
## L20260826-001 - datetime ticket-ID parsing: datetime-first regex, single-source const regexes, audit all consumers for case asymmetry (DIA-260826-pjm T1 fixes, 2026-08-26)

- Lesson: (1) the DIA-YYMMDD-xxxx ID regex must try the datetime form FIRST - a leading \d+ alternation greedily truncates datetime IDs; (2) single-source-of-truth const regexes beat inline literals: of 6 inline copies of the ID pattern, 4 had already drifted; (3) when fixing ID parsing, audit ALL consumers for case-normalization asymmetry - the F1 Blocker was scan side uppercasing while ledger side kept raw case, causing Path-1 hard-block on every datetime citation.
- Why irrecoverable: the drift/greediness/case-asymmetry failure modes are cross-file behavioral knowledge; each individual diff looks correct in isolation.
- Cross-reference: failures.md F1 Blocker entry (DIA-260826-pjm), DIA-234 ticket-ID formats.

## Reflect automation campaign (DIA-211 final lane, branch DIA-260822-medh-red, commits aef48498 + cd1eb7fd)

- PRETTIER-ROOT-CAUSE (S4 finding): an apparent "lost fix" in AGENTS.md was NOT a concurrent clobber - the DIA-105 edit-time formatter (prettier) canonicalizes multi-line code spans back to column 0. Durable fix = close the code span onto one line; verify formatter stability by re-running prettier after the edit. Lesson class: when a fix "went missing", check formatter canonicalization BEFORE blaming concurrent-session clobbering. Extends the prettier non-idempotency class (L20260815-001/002) with the edit-time (DIA-105) variant and the misdiagnosis trap.
- SCRIPT-OVER-PLUGIN for gate-flag clearing: pending-gate-clear was originally planned as a plugin hook; gate research BLOCKED plugin variants (hot-file collision with concurrently-modified plugin source + gate-weakening risk) in favor of a standalone script that ENCODES the same verification. Rule: verification gates must be cleared by tooling that RE-VERIFIES, never auto-expired; prefer a standalone script over a plugin hook when the plugin file is under concurrent modification.
- Meta-task ticket-gate governance (extends L20260820-003): besides minting a capability token for bootstrap, keep a PARENT EPIC ticket OPEN (e.g. DIA-211) to govern meta/analysis dispatches (reflect analysis, ticket creation for tooling work). Live evidence: meta-dispatches hard-blocked on missing ticket ID until governed by the open epic; documented in OPEN DIA-260820-jlu0.

## L20260826-002 - ai-auditor empty-result failure: @reviewer two-axis is an acceptable fallback independent reviewer for section 2.5 config changes (DIA-260826-spu5, 2026-08-26)

- Observation: in DIA-260826-spu5 the @ai-auditor independent-review lane returned EMPTY/blank final results TWICE consecutively (ai--2, ai--3) with session errors and no text. The developer chose @reviewer (two-axis: Standards + Spec fidelity) as the alternative independent reviewer for the section 2.5 opencode-config change. This is a SECOND distinct @ai-auditor failure mode after the ai--3 fabricated-review incident (failures.md ~L118-122 / lessons.md L41): @ai-auditor can fail both by fabricating content AND by returning empty results.
- Lesson: when @ai-auditor returns empty/blank results for a section 2.5 config independent-review step, @reviewer (two-axis review) is a viable alternative independent reviewer - its Standards + Spec-fidelity axes cover the independence requirement. This does NOT weaken the standing rule that Phase-6 independent review normally routes to @ai-auditor (lessons.md L94-95); it is the fallback when that lane is unavailable. Always follow the DIA-099 empty-result protocol (preserve the partial-results json in .opencode/session/partial-results/) before re-routing.
- Why irrecoverable: git shows only that @reviewer approved the change; it does not record that @ai-auditor failed empty twice nor that @reviewer was an accepted fallback. The cross-campaign pattern (ai-auditor failing in two modes) is not in any single ticket.
  - Cross-reference: DIA-260826-spu5, lessons.md L41 (ai--3 fabricated review), lessons.md L94-95 (ai-auditor = Phase-6 reviewer), DIA-099 empty-result protocol.

## L20260827-001 - WeezEngine research (DIA-260827-wez1): no public OpenCode tool exists; YAGNI - do not add now (2026-08-27)
...
## L20260901-001 - Fix committed but ticket never closed: post-commit bookkeeping gap (DIA-260827-y9n9 / DIA-260827-mgfv, 2026-09-01)

- Observation: DIA-260827-y9n9 (parallel-handoff slot hardening) and DIA-260827-mgfv (DIA-217 fail-closed gate) both had fix commits landed (54e2dc1, 93ea6ff) and tests passing, but the tickets remained OPEN in the ledger because the closing lane never executed. This is a recurring pattern (also seen in DIA-260820-jlu0 in a prior session).
- Distinguishing from L20260819-004: that entry covers README-index/status sync issues (status updated in ticket file but not README, or vice versa). This entry is specifically about the closing lane itself never running — no status update happened at all, in either the ticket file or the README. The root cause is different: L20260819-004 is a split-brain between two status sources; this is a single-source omission.
- Root cause: the workflow completes implementation + test verification but the "close ticket" step is a separate manual dispatch that can be forgotten or deferred. When a fix commit lands in-session, there is no automatic trigger that flips the ticket status.
- Preventive action: when a fix commit lands referencing a DIA ticket, the orchestrator should treat "commits referencing DIA-XXX exist but ticket still OPEN" as a normal post-commit bookkeeping step — close the ticket in the same pass or immediately after, not as a deferred separate step. Alternatively, add a post-commit hook or lint check that flags DIA-referencing commits whose tickets are still OPEN.
- Why irrecoverable: the fix commits and test outputs are in git, but the bookkeeping omission (closing lane never dispatched) is a workflow gap not visible in any commit. A fresh agent reading git log would see the fix landed and assume the ticket was closed.
- Cross-reference: L20260819-004 (adjacent, distinct: README-index drift vs omission), DIA-260820-jlu0 (same pattern in prior session).

## L20260901-002 - Completed/reconciled subagent sessions are NOT hand-resumable for a NEW task batch (2026-09-01)

- Observation: attempting to resume a completed coder session (ses_fa285459effeaPJJAHzLgGAUdq) for a NEW 20-ticket closure batch via `task(task_id=...)` was BLOCKED with "Idempotent duplicate dispatch blocked (agent: coder)"; `task_revive` on the same session failed with "relaunch lease unavailable". The Background Job Board lists completed sessions as "reusable by alias for the same specialist/context", but this reuse is board-routed (a fresh dispatch that the board may alias to the specialist type), NOT a manual `task_id` resume.
- Distinguishing from existing patterns:
  - The "Exact-instance resume success pattern" (lessons.md line 55) documents SUCCESSFUL resume for the SAME task/artifact continuation. That pattern applies when resuming a session to continue or fix the same work it was doing.
  - The "Same-session fixes rule (Q2)" (lessons.md line 839, DIA-175) reserves same-session reuse for the FIX loop only (implementer context needed for the same artifact).
  - THIS entry documents the FAILURE case: using `task(task_id=...)` or `task_revive` to hand-resume a completed session for a DIFFERENT/new task batch. The session is terminal; the board's "reusable" label means the specialist TYPE can be re-dispatched (via the board), not that the specific session can be revived for new work.
- Correct orchestrator behavior: for a NEW task batch (different objective, new set of tickets, unrelated work), dispatch a FRESH instance via the background job board. Do NOT attempt `task(task_id=...)` or `task_revive` on a completed session for new work. R4/R5 same-session reuse applies only to fix loops / same-artifact continuation on the SAME task_id, not to new batches.
- Preventive action: the orchestrator should treat "completed/reconciled" session status on the job board as TERMINAL for manual resume purposes. For new work, always dispatch fresh. The board's "reusable" label should be read as "this specialist type is available for dispatch", not "this specific session can be revived".
...
- Root cause: the gate did not detect the execution context. Inside poetry-dev, the pre-commit hook runs lint-staged directly (no Docker socket needed). On the host, the hook runs via docker compose exec dev. The gate assumed a Docker engine socket was always available, which is false inside the dev container.
- Preventive action / operational rule:
  1. Do NOT duplicate pre-commit verification logic in dispatch prompts. scripts/verify-pre-commit.sh is the single source of truth for pre-commit gating. It already detects context via hostname check (is_in_dev_container -> npx lint-staged directly; host -> docker compose exec dev with precise remediation on failure).
  2. When writing a commit-lane gate, detect context FIRST (hostname == "poetry-dev" means in-container; otherwise host). Only require Docker engine socket on the host path.
...
## L20260903-001 - empty-result transport truncation: a 4th empty-result variant distinct from silent-failure / reporting-artifact / partial-landing (DIA-260903-o7n0, 2026-09-03)

- Symptom: three lanes (cod-4 docs lane, resume lane, notes-resume lane) returned completely empty results in the F budget-gate campaign. Each time, ground-truth inspection via a dedicated read-only verification lane confirmed WORK_LANDED -- the work had fully landed (commits present, artifacts on disk). The empty result was a TRANSPORT-LEVEL truncation, not an agent-side empty delivery.
- Distinguishing from existing variants:
  - Failures.md line 260 (2026-08-14): silent-failure where nothing was written.
  - Failures.md line 441 (2026-08-27, DIA-260820): reporting-artifact where work landed but registry signature looked empty.
  - This variant (DIA-260903-o7n0): the agent DID produce output, but the transport layer (OpenCode task-result delivery) truncated it to empty before it reached the orchestrator. The agent-side evidence (what was produced) is lost; only the ground-truth on-disk state confirms landing.
- Operational rule: the 4th variant reinforces the verify-first ordering from the DIA-099 empty-result protocol. When ANY empty result is observed, ground-truth verification via a read-only lane is the ONLY reliable next step -- result-envelope inspection is insufficient because the envelope can be truncated by transport. The read-only lane must check: commit present, ticket status, artifact on disk, git status for staged/uncommitted edits. Apply BEFORE any decision to re-dispatch or accept.
- Why irrecoverable: the transport-truncation mechanism is a runtime delivery behavior; the commit diffs show only the landed work, not the empty envelope that the orchestrator received. Without this lesson, future orchestrators would misclassify transport-truncated empty results as silent failures and spawn unnecessary fresh writers.
- Cross-reference: DIA-099 empty-result protocol (the 3-variant taxonomy); failures.md line 260 (empty-result ambiguity, 2026-08-14); failures.md line 441 (registry-signature insufficiency, 2026-08-27); DIA-260903-o7n0 (F budget-gate campaign, 2350ee8 + 921394d + e3744fa + a1fffb2).

## L20260903-002 - RED-owned battery contract: RED test-author catches genuine gate bugs; GREEN fixture gaps reported by GREEN are closed by RED (DIA-260903-o7n0, 2026-09-03)

- Observation: in the F budget-gate campaign the RED test-author lane (c130692) built a hermetic 25-case bats battery. During extension (a2cca20, M4 campaign fixtures), RED discovered a genuine GATE-BUG (regression-5b): the gate script hard-failed on an unresolvable BUDGET_PLUGIN_ROOT when the plugin was absent. This was a real behavioral defect in the gate script, not a test artifact. GREEN (6549060) then fixed the gate to fail-closed on unresolvable root. Separately, GREEN reported M4 fixture gaps during its own battery run, and those gaps were closed by RED in the same extension commit.
- The contract: RED ownership of the test battery means RED is responsible for BOTH authoring test cases AND closing fixture gaps -- even when GREEN is the one that detects the gap. GREEN's role is to report the gap; RED's role is to fix the fixture and close the test. This ownership discipline prevented GREEN from inadvertently writing RED-authored tests (which would violate DIA-175 instance separation) and ensured the gate bug was caught by the test-authoring lane that understood the test's invariants.
- Generalizable rule: in any RED/GREEN split, test-fixture ownership stays with RED even when GREEN reports the gap. GREEN may report "fixture gap found" but must NOT author the fix. The RED test-author closes it, preserving instance separation and ensuring the fix understands the test's design intent.
- Why irrecoverable: the gate bug (5b) is a committed behavioral fact (a2cca20 + 6549060), but the ownership-discipline decision ("RED closes GREEN-reported gaps") is a workflow pattern not stated in any commit. Without this lesson, a future orchestrator would route GREEN-reported fixture gaps back to GREEN, violating DIA-175 and risking test-authoring under wrong ownership.
- Cross-reference: DIA-175 strict instance separation (RED/GREEN); DIA-260903-o7n0 RED ext commit a2cca20 (33/34, regression-5b reported); GREEN fix commit 6549060; RED cycle 2/2 commit 2b85757 (pre-manifest exemption + pins).

## L20260909-001 - `.prettierignore` excludes `.opencode/`; whitespace-only fixes in plugin test/lib files require manual targeted edits (DIA-260909-9c9x, 2026-09-09)

- Observation: `.prettierignore` covers `.opencode/`, so `prettier --write` on the repo will NOT touch files under `.opencode/plugins/__tests__/` or `.opencode/plugins/lib/`. Whitespace-only fixes (trailing spaces, blank-line indentation, extra EOF newlines) in these files cannot use the repo-wide prettier formatter. The fix lane must use manual targeted single-line edits only.
- Distinguishing from DIA-105 (edit-time formatter): DIA-105 documents that the delegation-observer plugin runs prettier on agent-edited files AFTER edit/write/apply_patch. That hook is PRETTIER-ONLY and scoped to files the agent edits. This entry is about a DIFFERENT constraint: `.prettierignore` excludes `.opencode/` entirely, so even `prettier --no-ignore` would reformat unrelated `.opencode` sources and pollute the diff. The manual-edit-only constraint applies to ANY whitespace fix in `.opencode/` plugin sources, not just agent-edited files.
- Operational rule: when fixing whitespace in `.opencode/plugins/` test or lib files, use manual targeted edits (one line per file). Do NOT run `prettier --write` or `prettier --no-ignore` on the repo to fix these files -- that would reformat unrelated `.opencode` sources and create a noisy diff. Verify with `git diff -w` (must be empty, confirming no non-whitespace delta) and `rg -n ' +$'` (trailing-WS count must be 0 per file).
- Why irrecoverable: the `.prettierignore` scope and the manual-edit constraint are not stated in any committed file or test. A fresh agent seeing whitespace issues in `.opencode/` plugin files would naturally reach for `prettier --write` and either get no effect (prettierignore) or reformat unrelated files (--no-ignore). The learnings file (external-patterns/2026-09-09-dia-260909-9c9x-whitespace-gate.md) captures the full gate analysis; this entry records the durable operational rule.
- Cross-reference: DIA-105 (edit-time formatter, adjacent distinct), `.prettierignore` (repo root), `.opencode/learnings/external-patterns/2026-09-09-dia-260909-9c9x-whitespace-gate.md`.

## L20260909-002 - Verbatim-move constraint for whitespace-only code edits: do not reorder or rewrap surrounding lines (DIA-260909-9c9x, 2026-09-09)

- Observation: when fixing whitespace-only issues inside code structures (e.g. blank-line indentation inside a `try` block), the fix MUST be a whitespace-only deletion of the target bytes. Do not reindent, rewrap, or reorder surrounding lines. The code semantics are unchanged by whitespace removal; any non-whitespace delta on the move is a defect.
- Concrete trigger: DIA-260909-9c9x Target 2 was 8 spaces on a blank line inside a `try` block in `.opencode/plugins/lib/stall-sweep.ts:263`. The fix was deletion of those 8 bytes only. Reindenting the surrounding `try` body or rewrapping adjacent statements would change line positions, potentially breaking source-map or line-number references, and would make `git diff -w` non-empty (a signal the fix is not whitespace-only).
...
- Distinguishing from existing lessons:
  - L20260816-003 and L20260818-002 document same-session fix-loop WORKING (session available, resume succeeds).
  - This lesson documents the FAILURE MODE: when the session is pruned from the reusable list (expired, garbage-collected, or otherwise unavailable), the same-session fix-loop is IMPOSSIBLE. The fallback is a fresh-session dispatch.
  - The critical detail: the fresh-session dispatch must carry the FULL context prompt (complete task spec, implementation state, and the specific fix needed). Abbreviated "continue previous work" prompts fail because the fresh instance has zero prior context.
