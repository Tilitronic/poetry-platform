...
- Failure mode (2026-08-27, DIA-260820 batch "Do all"): registry signature alone cannot discriminate silent-failure from reporting-artifact - ground-truth verify-first is mandatory
  - Symptom: the SPEC-AUTHORING batch hit TWO empty subagent results - a @coder fix turn (jcte) and a @openspec-plan spec turn (DIA-206). Both looked identical at the registry/result level (empty result_ref), so a signature-only check could not tell which was which.
  - Discrimination via ground truth (NOT signature): jcte empty was a REPORTING ARTIFACT - the work had landed in the working tree and was committed by the human as ea06ecd; DIA-206 empty was a GENUINE non-landing - the spec file was absent on disk, and a redispatch succeeded and produced it. The two cases required opposite handling (jcte: accept as done; DIA-206: redispatch), but the registry looked the same for both.
  - Lesson: a registry/result signature (empty result_ref, zero edits) is INSUFFICIENT to classify an empty subagent result. The only reliable discriminator is ground-truth verification (git status + working-tree + artifact-on-disk) via a read-only lane BEFORE deciding accept-vs-redispatch. This confirms and sharpens the 2026-08-14 empty-result ambiguity rule (failures.md line 260): the three variants (silent-failure / reporting-artifact / partial-landing) are distinguished ONLY by repo state, never by the result envelope.
  - Why irrecoverable: the classification decision and the verify-first ordering are session behavior; git shows only the eventual outcomes (ea06ecd landed; DIA-206 spec present after redispatch), not the empty-result detection and the ground-truth check that chose the handling.
  - Cross-reference: failures.md line 260 (empty-result ambiguity, 2026-08-14); DIA-099 empty-result protocol; DIA-206; commit ea06ecd (jcte human-authored closure); resume-truncated-lane skill.

- Failure mode (2026-09-03, DIA-260903-o7n0): cancel-receipt-but-running -- dispatches returning "Task cancelled" still executed under lifecycle ownership twice
  - Symptom: during the F budget-gate campaign, dispatches returned a "Task cancelled" status to the orchestrator, yet the underlying task continued executing under the lifecycle owner's session. The orchestrator, seeing the cancellation receipt, treated the lane as dead and dispatched fresh replacements -- resulting in duplicate execution of the same work (two instances running the same implementation in parallel).
  - Root cause: the task-result delivery reported "Task cancelled" before the lifecycle owner had actually stopped execution. The cancellation receipt is asynchronous with respect to actual task termination; the orchestrator's receipt of "cancelled" is NOT proof that the task stopped running. The lifecycle owner may still be in-flight when the cancel receipt arrives.
  - Preventive action: ALWAYS check task_status or task_result before retrying or re-dispatching a cancelled lane. A "Task cancelled" receipt alone is insufficient to assume the lane is dead. The correct sequence is: (1) receive cancellation receipt, (2) check task_status to confirm terminal state, (3) ONLY if confirmed terminal, proceed with re-dispatch. If status shows still-running, wait or check ground truth (artifacts on disk) before acting.
  - Distinguishing from existing patterns:
    - Failures.md line 71 (2026-08-04): dispatched-but-not-executed when batching -- the opposite direction (dispatch never ran). This variant: dispatch RECEIVED a cancel but DID run.
    - Failures.md line 395 (DIA-260825-nts7): task_revive rejected with lifecycle-ownership error -- revive blocked. This variant: the cancel receipt was accepted but the task was still running.
  - Why irrecoverable: the duplicate execution is a race condition between cancellation receipt delivery and actual task termination; git diffs show only the final state (one commit), not the parallel execution window where two instances wrote the same files. The "check status before retry" rule is operational discipline not stated in any committed file.
  - Cross-reference: failures.md line 71 (batched dispatch drop); failures.md line 395 (revive lifecycle-ownership error); DIA-099 empty-result protocol; DIA-260903-o7n0 (F budget-gate campaign).

- Failure mode (2026-09-03, DIA-260903-o7n0): 3-cancel dispatch-block episode -- orchestrator dispatch blocked by cancellations 3 times, wait-and-retry recovery
  - Symptom: during the F budget-gate campaign, the orchestrator attempted to dispatch a task and was BLOCKED by a "Task cancelled" status. The orchestrator retried and was blocked 2 more times (3 total cancellations). Each cancellation blocked the dispatch from proceeding -- the task never ran under the cancelled dispatch. The recovery was to WAIT (let the lifecycle owner finish or the cancellation queue drain) and then RETRY the dispatch successfully.
  - Distinguishing from cancel-receipt-but-running (failures.md line 448): that entry covers the case where the dispatch RETURNED "Task cancelled" but the task STILL RAN under lifecycle ownership (duplicate execution). This entry covers the OPPOSITE case: the dispatch was genuinely BLOCKED by cancellations (the task never ran), and the recovery required waiting and retrying.
  - Root cause: the cancellation queue or lifecycle ownership created a window where new dispatches were blocked until the prior cancellation resolved. The orchestrator could not proceed immediately; a retry-without-wait would hit the same block.
  - Preventive action: when a dispatch is blocked by cancellation, DO NOT immediately re-dispatch (retry-without-wait hits the same block). Instead: (1) check task_status to confirm the prior task is terminal, (2) if still in-flight, WAIT before retrying, (3) retry with the full task context (do not abbreviate the re-dispatch payload -- the fresh instance has no prior context). The wait-and-retry pattern avoids the 3-cancel loop.
  - Why irrecoverable: the 3-cancel count, the blocking behavior, and the wait-and-retry recovery are runtime/session observations not stated in any commit or ticket. Git shows only the final landed work, not the 3-block-then-retry sequence that produced it.
  - Cross-reference: failures.md line 448 (cancel-receipt-but-running, adjacent distinct pattern); failures.md line 395 (revive lifecycle-ownership error); DIA-099 empty-result protocol; DIA-260903-o7n0 (F budget-gate campaign).

- Failure mode (2026-09-09, DIA-260909-zeik): routing gate blocked coder before ai-specialist gate could run
  - Symptom: the section-10 routing gate (DIA-230) detected a config-work keyword match in the coder dispatch and emitted a ROUTING_VIOLATION warning, because the coder dispatch referenced a file path pattern that matched `.opencode/plugins/__tests__/` before the ai-specialist gate had run in the session. The gate advisory blocked the coder dispatch.
  - Root cause: the routing gate's keyword/path-match triggered on the coder's dispatch payload before the ai-specialist research gate had fired. The gate is advisory (warning, not hard-block) but the orchestrator treated it as a signal to pause and route through ai-specialist first.
  - Recovery: the ai-specialist gate research ran first (producing the learnings file dia-260909-zeik-scenario-cleanup-gate.md), then the coder was dispatched with gate clearance.
  - Preventive action: when dispatching @coder on work that touches `.opencode/plugins/__tests__/` paths, run the @ai-specialist gate research FIRST to satisfy the routing gate's keyword match. Do not dispatch @coder on config-adjacent paths without prior ai-specialist research in the session.
  - Why irrecoverable: the routing gate's keyword-match behavior and the "run ai-specialist first" workaround are runtime plugin behavior not stated in any committed file; the dispatch sequence (ai-specialist -> coder) is a workflow decision not visible in the final commits.
  - Cross-reference: DIA-260909-zeik, DIA-230 (routing-order gate), delegation-observer.ts routing gate, .opencode/learnings/external-patterns/dia-260909-zeik-scenario-cleanup-gate.md.

- Failure mode (2026-09-09, DIA-260909-zeik): budget has_backing fail-closed on CLOSED o7n0 manifest entries required manifest fix
  - Symptom: the budget-gate commit-msg hook (`scripts/check-budget-gate.sh`) resolved has_backing=false because the parent epic DIA-260903-o7n0 was CLOSED in the manifest. CLOSED ticket entries do not provide backing for OPEN ticket work. The gate failed-closed, blocking the commit.
  - Root cause: the gate's has_backing resolution checks for OPEN entries matching the Budget-Scope; CLOSED entries are intentionally excluded (CLOSED entries represent completed work and must not authorize new work under the same scope). The o7n0 ticket was CLOSED, so its manifest entries provided no backing.
  - Recovery: a ruling-authorized one-line manifest addition added a test-debloat entry for OPEN DIA-260909-zeik, satisfying the gate. The manifest entry for o7n0 remained CLOSED.
  - Preventive action: when a budget-scope commit references a CLOSED parent epic, the fix is a ruling-authorized manifest addition for the OPEN child ticket, not reopening the parent or bypassing the gate. The gate's fail-closed behavior is correct by design.
  - Why irrecoverable: the gate's fail-closed behavior and the ruling-authorized manifest fix are runtime/plugin behavior and process decisions not stated in any committed file; the manifest diff shows the one-line addition but not the CLOSED-entry exclusion logic or the ruling authorization.
  - Cross-reference: DIA-260909-zeik, DIA-260903-o7n0, scripts/budget-baselines.json, scripts/check-budget-gate.sh, adr.md "Budget gate fail-closed" ADR.

- Failure mode (2026-09-09, DIA-260909-zeik): prettier N/A on ignored path -- plugins/__tests__/ is in .prettierignore
  - Symptom: the prettier format check was proposed as a verification gate for the scenario-cleanup changes, but `.opencode/plugins/__tests__/` is covered by `.prettierignore` in the repo root. The prettier check was N/A (not applicable) for these files.
  - Root cause: `.prettierignore` excludes `.opencode/` entirely, so `prettier --check` on the repo will NOT touch files under `.opencode/plugins/__tests__/`. The format gate is irrelevant for this file set.
  - Preventive action: when proposing prettier as a verification gate for changes under `.opencode/`, first check `.prettierignore` for whether the path is excluded. If excluded, skip the prettier gate and note the exclusion reason. Do not fail a gate that is N/A by design.
  - Why irrecoverable: the `.prettierignore` exclusion is a repo configuration fact; the N/A determination is a process observation not stated in any committed file or ticket.
  - Cross-reference: DIA-260909-zeik, .prettierignore, L20260909-001 (prettierignore scope lesson).

- Failure mode (2026-09-10, DIA-260909-9i1o): 3-test double-prefix miss during bulk-rename of fixture helpers
  - Symptom: during consolidation of budget-gate fixture functions from suite-local helpers into test-helper.bash, a bulk rename (e.g. `write_manifest` -> `budget_write_manifest`) left 3 call sites in budget-gate.bats with incomplete or double-prefixed names. The reviewer's rev-1 Standards axis caught these as defects before merge.
  - Root cause: the bulk rename was applied per-file using find-and-replace, but the grep for remaining old-name references did not account for the new prefix already being present in some contexts (comments, partial renames). Some files were partially renamed before the grep ran, leaving stale references that a single-pass rename missed.
  - Preventive action: when bulk-renaming fixture functions with a namespace prefix, use a two-pass verification: (1) grep for the OLD name to catch remaining references, (2) grep for double-prefix patterns (e.g. `budget_budget_`) to catch over-renames. Do not rely on a single pass of find-and-replace. The rename is not complete until both greps return zero matches.
  - Why irrecoverable: the 3-test miss was caught by reviewer Standards inspection, not by any automated check. The final commit (89ce9fc) contains only the corrected names; the double-prefix misses and their detection are review-process behavior not visible in the diff. Without this lesson, future bulk renames will repeat the same single-pass mistake.
  - Cross-reference: DIA-260909-9i1o, commit 89ce9fc, reviewer rev-1 Standards findings, L20260909-9i1o-002 (bulk-rename double-prefix lesson).

- Failure pattern (2026-08-27, DIA-260827-36ht): reviewer falsification checks caught non-executing behavioral tests (fake-Docker recording without execution)
  - Symptom: the DIA-189 desktop-toast behavioral tests were wired into the pre-push gate but the test harness recorded pass results without actually executing the tests. The tests used a fake-Docker recording pattern (mocking child_process spawn) that captured expected output without running the real behavioral assertions. The reviewer's falsification checks detected that the tests never exercised the actual platform-detection or toast-sending code paths.
  - Root cause: behavioral test stubs that mock external dependencies (child_process spawn, fs) can produce "passing" output that looks like real test execution but never invokes the code under test. The mock output is deterministic (hardcoded expected values) so it always matches, creating a false-green signal.
  - Preventive action: behavioral test pins MUST prove execution, not just recording. A test that mocks the dependency under test and asserts against mock output is testing the mock, not the code. Verification discipline: for behavioral tests, assert on SIDE EFFECTS (file state changes, spawned process arguments, error propagation) rather than on mock-captured output. The reviewer falsification check pattern: grep for assertions against mock-captured variables vs assertions against real execution traces.
  - Why irrecoverable: the fake-Docker recording pattern and the reviewer falsification detection are test-architecture knowledge not stated in any committed test file; the test diff shows the mock setup but not that it was testing the mock rather than the code.
  - Cross-reference: DIA-260827-36ht, DIA-189, openspec/changes/dia-260827-36ht-plugin-behavioral-gate/design.md D2.

- Failure mode (2026-09-10, DIA-260831-c3d4): H1 lint-staged DETERMINISTIC failure on Prettier-DIRTY markdown -- stash/restore merge-conflict in hook (verified 2x)
  - Symptom: committing a Prettier-DIRTY .md file (any markdown whose formatting differs from prettier's output) triggers lint-staged's auto-restage workflow. The hook stashes unstaged changes, runs prettier --write on staged files, then attempts to restore the stash. The restore produces a MERGE CONFLICT and the commit aborts with exit 1. This is DETERMINISTIC: 2x confirmed FAIL when DIRTY, 2x PASS when CLEAN.
  - Distinguishing from existing entries: this is NOT the partial-staging conflict variant (failures.md line 71-73). That variant requires partially staged hunks. THIS variant fires on FULLY-STAGED files whose content is Prettier-DIRTY -- no partial staging required. The hook's stash/restore lifecycle is the trigger.
  - Root cause: lint-staged's stash-backup + restore workflow conflicts with prettier's reformatting when the staged file content differs from prettier's output. The stash captures the pre-format state; after prettier --write modifies the staged files, restoring the stash produces a conflict because the working tree has been altered.
  - Fix that works: run `prettier --check <file>` BEFORE staging. If check fails, run `prettier --write <file>` BEFORE `git add`. This prevents the conflict because the file is already Clean when the hook runs. Verified 2x: DIRTY -> FAIL, CLEAN -> PASS.
