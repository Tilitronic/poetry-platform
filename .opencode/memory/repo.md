...
- origin/omo-slim-changes HEAD (2026-08-15): origin/omo-slim-changes advanced from
  ...
  origin at 80a148b. The origin move reflects DIA-190/192/193 config fixes + DIA-192
  ticket prettier-stable fix + DIA-182 live-smoke skip (commits 80a148b/41b33d2/
  6d1013d/f43f2e0 on top of the session-start base). Recorded because a fresh session
  reading only commit messages cannot reconstruct the before/after origin delta of
  the session window.

- CHANGELOG format (DIA-194/196, 2026-08-16, commit 8cade8c + ef0154a): the
  changelog source of truth is now the YAML ledger `.opencode/CHANGELOG.yaml`
  (96 entries as of 2026-08-17, reverse-chronological; count grows with each
  registration - verify via the ledger). `.opencode/CHANGELOG.md` is a DERIVED VIEW
  regenerated by `scripts/changelog-render` - never edit the MD by hand. New
  entries are appended to the YAML via a text edit (the settled PyYAML path, NOT
  `yq -i` - yq was rejected by DIA-137 and is not installed; the dead yq branch
  was removed from AGENTS.md 2.5 Phase 7). Partial reads use
  ...
  (prefix/startswith form also valid for multi-ticket entries); the orchestrator
  (bash: deny) delegates such lookups to a coder lane rather than pasting the
  full ~238 KB file. Schema + validation: `scripts/schemas/changelog.schema.json`
  + `scripts/validate-changelog.sh`, wired into `make test-config`. This format
  change is cross-file operational state (prompt instructions + render tooling +
  YAML ledger) not reconstructible from any single commit.

- knowledge/ana026-artifact-format-substrate/ is the DIA-194 analysis
  (report `ana026-artifact-format-substrate-report.md`), RENUMBERED from an
  earlier `knowledge/ana022-artifact-format-substrate/` untracked dir because a
  concurrent DIA-180 lane had already claimed `knowledge/ana022-worktree-mechanism-analysis/`
  as canonical (knowledge-ID collision, resolved 2026-08-16, commit afd6315). The
  DIA-180 sibling analysis lives at `knowledge/ana022-worktree-mechanism-analysis/`.
  The ana022 numeric slot now belongs to DIA-180; DIA-194 is ana026. Recorded
  because the renumber rationale (concurrent canonical-claim collision) is a
  session-time event not reconstructible from the renamed directory alone.

- context_usage tool semantics (DIA-191, 2026-08-17): the delegation-observer
  context_usage tool now performs a DIRECT LIVE IN-CONTEXT READ via the plugin
  SDK - the same computation the TUI indicator performs (last completed
  assistant message's tokens input+output+reasoning+cache.read+cache.write
  divided by model limit.context), compaction-aware by construction. The V1
  proxy formula (ana025 reweight) remains as the fallback for fresh sessions /
  client failure. The implementation is in
  .opencode/plugins/delegation-observer.ts (recoverable from code); this
