...
## Teaching Checkpoint (Phase 5 Operator Ownership)

**Before applying any Phase 4 self-improvement change:**

1. **Invoke `teaching`** to produce a short explanation of:
   - why this pattern occurs (the mechanism, not just the symptom)
   - why this specific fix addresses it (mental model, not just "do X")

2. **Attach the visualization** from `analyzer`'s `analysis-report.md` (table/tree/Mermaid) alongside the explanation — not just the raw diff.

3. **Presentation order:** visualization → teaching explanation → diff → confirmation prompt. The person should understand the "why" before seeing the "what".

4. **Check `.opencode/practice-protected.md`** before applying. If the change affects a practice-protected zone, do not apply automatically — ask the person to draft the artifact themselves.

## Practice-Protected Zones

The file `.opencode/practice-protected.md` defines zones where the agent asks guiding questions and waits for the person's draft instead of producing the artifact directly. Check this file before applying any change to those zones.
