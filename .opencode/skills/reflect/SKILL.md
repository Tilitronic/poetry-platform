...
### 6. Synthesis Step (Phase 4 Self-Improvement Loop)

When `analyzer` has produced an `analysis-report.md` with a quantitative pattern:

1. **Check `.opencode/learnings/index.md`** for a prior finding on this agent/skill/pattern.
   - If found and marked "✅ confirmed effective" → apply the same fix directly, skip to step 4, no new RAG query needed.
   - If found and marked "⚪ no effect" or "🔴 regressed" → do not repeat the same fix; treat as unresolved and continue to step 2.

2. **If no prior finding**, formulate a `#rag` query to `book-rag` on the pattern's topic (orchestration, delegation, error recovery, prompt design, model routing, etc.).

3. **If `book-rag` returns a relevant approach:**
   - Write `.opencode/learnings/[date]-[topic].md`:
     - the pattern (from analyzer's report, with numbers)
     - the RAG source (citation + book/chapter)
     - the proposed change (specific config/prompt/skill file)
     - outcome: "pending" (filled in at outcome-verify stage)
   - Update `index.md` with a one-line pointer to this entry.
   - Update `CHANGELOG.md` with the entry.

4. **If `book-rag` returns nothing relevant:**
   - Record the pattern in `unresolved.md` without proposing a change.

5. **Output the proposed diff in chat.** Do NOT apply automatically. Present for user confirmation with:
   - the pattern evidence (from analyzer's report)
   - the RAG source (if found)
   - the specific config/prompt/skill change proposed
   - which metric this change is meant to move

6. **After user confirmation**, the `oh-my-opencode-slim` skill applies the change.
   Before applying, produce a short human-readable summary of what changed and why, reference the relevant metric from analyzer's report, and record in `CHANGELOG.md`.

7. **Outcome verification** (runs N sessions after the change was applied):
   - Re-run analyzer's telemetry query on the same metric.
   - Compare before/after:
     - Metric improved beyond noise → mark "✅ confirmed effective"
     - No measurable change → mark "⚪ no effect"
     - Metric got worse → mark "🔴 regressed" and flag for reverting
   - Update the outcome field in the learnings entry and `index.md`.
