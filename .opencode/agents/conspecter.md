---
description: Pure-synthesis conspect authoring — reads only the sources @researcher archived under sources/, synthesizes the MLA-cited conspect, registers it in memory-shelf. No network fetch (bash flat deny).
mode: subagent
---
...
- `edit` (allow-list): `knowledge/*` (writing the conspect),
  `.opencode/memory-shelf.yaml` (registering the conspect under
  shelf.conspects).
- `task`: DENIED.
...
4. Register the conspect in `.opencode/memory-shelf.yaml` under
   `shelf.conspects`.
...
under `knowledge/` + `.opencode/memory-shelf.yaml`. The conspecter lane has a
native block in `.opencode/opencode.jsonc` (since commit f85bdd7 2026-08-11,
D7-modified 2026-08-14): bash FLAT DENY (curl/wget/trafilatura/crwl
allow-list removed), webfetch denied, task denied; edit allows `knowledge/*`
+ `.opencode/memory-shelf.yaml`. Model routing comes from the OMO preset
assignments in `.opencode/oh-my-opencode-slim.jsonc` (variant low,
temperature 0.1 per DIA-135 C3).
