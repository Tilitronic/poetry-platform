...
// Agent descriptions for the boss prompt
...
const VALIDATION_ROUTING = [
  ...
  '- Route code review, code simplification and maintainability review checks to @reviewer',
  '- Route architecture validation to @architector',
  '- Route implementation to @coder or multiple @coder instances for maximum parallel execution',
...
const PARALLEL_DELEGATION_EXAMPLES = [
  '- Multiple @code-navigator searches across different domains?',
  '- @code-navigator + @researcher research in parallel?',
  '- Multiple @coder instances for faster, scoped implementation?',
  '- @observer + @code-navigator in parallel (visual analysis + code search)?',
];

/**
 * Build the boss prompt with dynamic agent filtering.
 * @param disabledAgents - Set of disabled agent names to exclude from the prompt
 * @returns The complete boss prompt string
 */
export function buildBossPrompt(disabledAgents?: Set<string>): string {
...
- Launch specialist agents in the background by default so the boss stays unblocked and can reconcile results when they return.
- Track each task's specialist, objective, task/session ID, and file/topic ownership.
- Continue boss work only on non-overlapping work; otherwise briefly report what was launched and stop.
...
- The boss should review and improve user-facing copy after designer work, because designer copy may be weak.
- Copy edits must preserve the designer's visual structure and interaction intent.
- If follow-up work is purely mechanical and preserves the design exactly, @coder can handle it. If it requires visual judgment or changes the feel, route it back to @designer.
...
- If the Background Job Board lists \`cod-1 / ses_abc / coder\`, call task with \`subagent_type: "coder"\` and \`task_id: "cod-1"\` or \`task_id: "ses_abc"\`.
...
export function createBossAgent(
  model?: string | Array<string | { id: string; variant?: string }>,
  customPrompt?: string,
  customAppendPrompt?: string,
  disabledAgents?: Set<string>,
): AgentDefinition {
  const basePrompt = buildBossPrompt(disabledAgents);
  const prompt = resolvePrompt(basePrompt, customPrompt, customAppendPrompt);

  const definition: AgentDefinition = {
    name: 'boss',
    description:
      'AI coding boss that delegates tasks to specialist agents for optimal quality, speed, and cost',
    config: {
      temperature: 0.1,
      prompt,
    },
