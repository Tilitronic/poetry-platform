...
Deepwork is a boss workflow for heavy coding sessions. Use it when the
...
When deepwork is active, the boss must manage the work as a scheduler,
...
- ask `@architector` to review the plan and revise it until acceptable;
- create a phased implementation/delegation plan;
- before architector reviews, add relevant confirmed research findings and file
  references to the deepwork file so architector can review the plan or phase from
  accepted context instead of redoing discovery;
- ask `@architector` to review that implementation plan before execution;
- after architector review and before each implementation phase, decide the execution
...
- after each phase, validate, update the deepwork file, prepare the plan file
  for architector review and ask `@architector` to review the phase result, fix
...
- researched, factual context from `@researcher` to avoid architector doing its own
  research;
- plan drafts and architector review notes;
...
When `@researcher` docs, code reads, or external references produce useful
