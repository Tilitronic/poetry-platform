...
## Responsibility
Manages the cloning and management of read-only dependency source repositories into `.slim/clonedeps/repos/` for offline inspection. This skill provides a workflow (not a command wrapper) that guides the boss and `@researcher` through cloning dependency sources so agents can inspect library internals without requiring network access.

## Design
- **Workflow skill, not a command wrapper**: No helper scripts or TypeScript utility functions. The boss owns the decision-making; `@researcher` recommends sources; the boss performs filesystem/git operations directly.
...
2. **Ask researcher for plan**: Delegate dependency discovery and source resolution to `@researcher`, who returns a small plan (dependency name, repo URL, ref, package subdirectory, reason).
...
## Integration
- **Consumed by**: Agents that need to inspect dependency internals (e.g., `@researcher`, `@code-navigator`) via the registered paths in AGENTS.md.
