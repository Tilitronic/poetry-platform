---
name: clonedeps
description: Clone important project dependency source code into an ignored local workspace so OpenCode can inspect library internals. Use when the user asks to clone dependencies, inspect dependency/source internals, understand SDK/framework behavior from source, debug library implementation details, or make core dependency repos locally readable. Do not use for ordinary API/docs questions where @researcher is enough.
---
...
boss and `@researcher` do the repo-specific thinking; the orchestrator
...
1. Read it before asking researcher for a new plan.
...
4. Only ask researcher for new recommendations if the existing manifest is
...
Delegate dependency discovery and source resolution to `@researcher`.
...
The boss owns final approval. Before cloning:

1. Verify refs manually where possible with `git ls-remote`.
2. Prefer pinned tags or commit SHAs. If no exact tag exists, ask researcher to
