...
It is boss-owned. The boss delegates source discovery and URL/ref
resolution to `@researcher`, asks for approval, then performs the git and
filesystem operations directly.

There is intentionally no helper script. Dependency discovery, ref validation,
and cloning are repo-specific enough that the boss/researcher workflow is
...
metadata. After cloning, the boss should add or update a concise
...
before asking `@researcher` for new recommendations.
