...
function activationPrompt(task: string): string {
  return [
    ...
    '- draft a plan and get `@architector` review before implementation;',
    '- create and review a phased implementation/delegation plan;',
    '- execute phase by phase with background specialists where useful;',
    '- wait for hook-driven background completion, reconcile results, validate, and ask `@architector` to review each phase;',
    '- ask `@architector` to include simplify/readability feedback in phase reviews;',
