...
DELEGATION SCOPE (re-review 1/2, ai--4 item 5): the orchestrator has
bash: deny and no direct read access to .opencode/CHANGELOG.yaml, so it
MUST NOT run the lookup commands itself. The partial-read lookup is
executed BY A DELEGATED CODER LANE (any agent whose permission block
allows bash + read of the ledger - @coder, @code-navigator for a quick
recon): the orchestrator dispatches the exact query below, the lane runs
it and returns only the matching entry text. The orchestrator never pastes
the full file into a prompt; it only forwards the query + ticket id.

Lookup commands (executed by the delegated lane):

    yq '.[] | select(.ticket == "DIA-NNN")' .opencode/CHANGELOG.yaml

For an entry whose ticket spans multiple tickets (e.g. "DIA-190/192/193"),
match the prefix instead: `select(.ticket | startswith("DIA-190"))`. If yq is
not installed on the host (DIA-137 rejected yq; host may lack the binary),
use the PyYAML equivalent that works everywhere the validator runs -
exact-match and prefix-match forms (prefix parity with the yq startswith
form, re-review 1/2 ai--4 item 6):
