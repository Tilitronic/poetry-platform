...
**Exit checksum delegation (DIA-093, FIX E):** Before writing the handoff file at cycle
termination, dispatch a coder lane to compute the SHA256 checksum of the prognosis per
the DIA-061 canonical serialization; write the returned 64-hex value into the `checksum`
field. NEVER write `checksum: null` unless the exit is a crisis/crash where coder dispatch
is impossible - in that case `resume_instructions` MUST explicitly flag `lane-0 checksum
delegation required`.
...
2. **Note checksum state (verification delegated, DIA-093).** The DIA-061 checksum is
   ...
   - on MISMATCH after lane-0 computation (stored 64-hex value differs from the computed
     ...
     content_ref: 'checksum-mismatch', prognosis recording computed= vs stored=);
...
5. **Log** the gate via the `log_decision` tool (event_type: 'handoff' at
   detection, task_ref: 'batch-approval-gate'; event_type: 'decision',
   resolution_status: 'acknowledged', content_ref: 'batch-approval-complete'
   once all items are approved). The
