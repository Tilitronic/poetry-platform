...
available and changes the boss from a primary worker into a scheduler.

The old model was:

```text
boss works directly → delegates when useful → waits for result
```

The default background-orchestration model is:

```text
boss plans → dispatches background specialists → monitors → reconciles → verifies
```
...
The boss is not the default implementation worker.
...
Specialists do the work. The boss manages the work.
...
The boss should not act on assumptions from a still-running task. It can
...
Before dispatching agents, the boss identifies:
...
The boss records the returned task ID and keeps working only on safe,
...
The boss should use background completion events to:
...
The boss should use `cancel_task` only when the user asks, or when a
...
as reconciled means its terminal result was injected into an boss turn
that completed and the parent returned to idle; it is not proof the result was
explicitly acknowledged or used. The boss should still verify it consumed
the relevant outputs before finalizing.

Specialist outputs are inputs, not final truth. The boss reconciles them
against each other and the original user goal.

### 5. Verify

Verification remains boss-owned, but not necessarily boss-run.
...
Visual/media analysis isolated from the boss context.

---

## Direct Work Boundary

Background orchestration removes the boss-as-worker default.

The boss may directly:
...
ownership need to be explicit in the boss's working context.
...
multiplexer panes attached while the parent boss continues scheduling.
...
The boss should do something like:
...
At no point does the boss become the main implementer.
...
- the boss launches independent specialists in background by default,
...
- the boss context stays focused on decisions instead of worker detail.
