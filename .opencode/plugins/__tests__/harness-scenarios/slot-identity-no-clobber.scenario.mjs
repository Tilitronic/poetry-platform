...
await runScenario("c5-s3-", async ({ directory, fail }) => {
  ...
  if (!existsSync(slotAPath)) fail("ses_pre_A.json does not exist")
  if (!existsSync(slotBPath)) fail("ses_pre_B.json does not exist")
  if (existsSync(unknownPath)) fail("unknown.json exists -- slot identity fell back to 'unknown'")

  // Each slot carries its own prognosis and a valid checksum.
  const slotA = readJson(slotAPath)
  const slotB = readJson(slotBPath)

  if (slotA.session_id !== "ses_pre_A") fail(`ses_pre_A.json session_id=${slotA.session_id}, expected ses_pre_A`)
  if (slotA.prognosis.resume_instructions !== "resume from session A") fail("ses_pre_A.json prognosis mismatch")
  if (slotA.checksum !== canonicalChecksum(slotA.prognosis)) fail("ses_pre_A.json checksum mismatch")

  if (slotB.session_id !== "ses_pre_B") fail(`ses_pre_B.json session_id=${slotB.session_id}, expected ses_pre_B`)
  if (slotB.prognosis.resume_instructions !== "resume from session B") fail("ses_pre_B.json prognosis mismatch")
  if (slotB.checksum !== canonicalChecksum(slotB.prognosis)) fail("ses_pre_B.json checksum mismatch")

  // Pointer points to the most recent writer (ses_pre_B).
  const pointer = readJson(pointerPath)
  if (pointer.active_session_id !== "ses_pre_B") fail(`active.json active_session_id=${pointer.active_session_id}, expected ses_pre_B`)
  ...
  if (slotFiles.length !== 3) fail(`expected 3 .json files in handoffs/, got ${slotFiles.length}: ${slotFiles.join(", ")}`)
})
