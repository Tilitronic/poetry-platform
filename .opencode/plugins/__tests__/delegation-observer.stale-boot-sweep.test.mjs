...
function freshCtx() { const { directory, cleanup } = createTempWorkspace("dia260822-fksf-")
  workspaceCleanups.push(cleanup)
  return { directory,
    client: { app: { log: async () => {} } }, } }

function registryPath(directory) { return join(directory, ".opencode/session/registry.jsonl") }

function readRegistryRows(directory) { const p = registryPath(directory)
  if (!existsSync(p)) return []
  return readFileSync(p, "utf-8")
    .trim().split("\n").filter(Boolean).map((l) => { try { return JSON.parse(l) } catch { return null } }).filter(Boolean) }

function countStallDetected(directory) { return readRegistryRows(directory).filter((r) => r.event === "stall_detected").length }

function countDeadDetected(directory) { return readRegistryRows(directory).filter((r) => r.event === "stall_detected" && r.escalation === "dead").length }

function appendRawRow(directory, row) { appendFileSync(registryPath(directory), JSON.stringify(row) + "\n") }
...
function captureSweepInstall() { let captured = null
  ...
  globalThis.setInterval = function(fn, ms, ...args) { if (ms === 60_000) captured = fn
...
async function makeHarnessWithSweep(bootTimeMs) { const dateMock = mockDateNow(bootTimeMs)
  ...
  return { ctx,
    ...
    sweepCapture,
    prevBootFlag,
    async cleanup() { try { await hooks.dispose() } catch { void 0; }
...
describe("DIA-260822-fksf stale stall-sweep startup protection (RED)", () => { test("AC1: N=390 stale nonterminal keys - first sweep emits ZERO stall_detected", async () => { const bootTime = Date.now()
    ...
    const { directory, cleanup } = createTempWorkspace("dia260822-fksf-ac1-")
    workspaceCleanups.push(cleanup)
    for (let i = 0; i < 390; i++) { seedNonTerminalKey(directory, `ses_stale_${i}`, staleTs) }
    ...
    try { // Advance to 15 minutes after boot so stale age = 24h+15m > threshold
      ...
      expect(emitted).toBe(0) } finally { try { await hooks.dispose() } catch { void 0; }
      const h = globalThis[STALL_SWEEP_KEY]
      if (h !== undefined) { try { clearInterval(h) } catch { void 0; } }
      globalThis[STALL_SWEEP_KEY] = undefined
      sweepCapture.restore()
  ...
  test("AC2: fresh key (timestamp after load) emits once its role threshold is crossed", async () => { const bootTime = Date.now()
    const h = await makeHarnessWithSweep(bootTime)
    try { expect(h.sweepFn).not.toBeNull()
      ...
      expect(emitted).toBe(1) } finally { await h.cleanup() } })

  test("AC3: key that becomes nonterminal DURING current lifetime is detected on subsequent sweep", async () => { const bootTime = Date.now()
    const h = await makeHarnessWithSweep(bootTime)
    try { expect(h.sweepFn).not.toBeNull()
  ...
  test("AC4: dead-escalation path unaffected for current-lifetime key crossing 60-min deadline", async () => { const bootTime = Date.now()
    const h = await makeHarnessWithSweep(bootTime)
    try { expect(h.sweepFn).not.toBeNull()
      ...
      expect(stalls).toBeGreaterThanOrEqual(1) } finally { await h.cleanup() } })

  test("AC5 regression: 390 stale + 1 fresh -> exactly 1 stall_detected", async () => { const bootTime = Date.now()
    const staleTs = new Date(bootTime - 24 * 60 * 60 * 1000).toISOString()
    const freshTs = new Date(bootTime + 1000).toISOString()
    // Seed 390 stale BEFORE boot (snapshot path) + 1 fresh
    const { directory, cleanup } = createTempWorkspace("dia260822-fksf-ac5-")
    workspaceCleanups.push(cleanup)
    for (let i = 0; i < 390; i++) { seedNonTerminalKey(directory, `ses_stale_${i}`, staleTs) }
    ...
    try { // Advance to 15m after boot, seed fresh then sweep
      dateMock.set(bootTime + 15 * 60 * 1000)
      ...
      expect(emitted).toBe(1) } finally { try { await hooks.dispose() } catch { void 0; }
      const h = globalThis[STALL_SWEEP_KEY]
      if (h !== undefined) { try { clearInterval(h) } catch { void 0; } }
      globalThis[STALL_SWEEP_KEY] = undefined
      sweepCapture.restore()
