...
// Commands that MUST be skipped — snip fork/exec fails on shell builtins.
// The plugin was already doing this for cd, source, export, etc.
// Extended with builtins we hit in practice.
const SKIP_SNIP = new Set([
  // posix builtins (no binary — snip fork/exec fails)
  "cd", "source", ".", "export", "alias", "unset", "set", "shopt",
  "eval", "exec", "echo", "true", "false", "type", "read",
...
function snipCommand(command: string): string {
  ...
  if (SKIP_SNIP.has(bareCmd.split(/\s+/)[0])) return command
