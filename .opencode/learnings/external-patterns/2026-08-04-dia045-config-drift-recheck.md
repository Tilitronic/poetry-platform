...
- **RESOLVED (2026-08-04, lane A commit)** — F18: vestigial `explorer` agent block removed from `.opencode/opencode.jsonc` (was :161-163; no model assigned in any preset; lane covered by `code-navigator` in all 3 presets; `disabled_agents` retains `explorer` to suppress the native OMO alias). F21: `compaction.reserved` 10000 → 16000. Committed as one change set with the DIA-045 ticket + this learning file. Supersedes the two OPEN entries above.
...
- **Executed (2026-08-04, lane A commit)** — F18 explorer cleanup + F21 compaction.reserved → 16000 (the two "Convenient" items above), committed with the DIA-045 F18/F21 change set.
...
- **Deferred items unchanged:** F18/F21/F15/F19/F20/F22 remain OPEN.
  - **2026-08-04 (lane A commit):** F18 + F21 now RESOLVED (see disposition above); remaining OPEN: F15/F19/F20/F22 + audit gaps.
- **Pending:** 4 post-restart smoke items (observer dispatch kimi, HANDOFF gate on template-produced HANDOFF, @ai-specialist/@resource-manager name resolution, coder escalation "orchestrator").
  - **2026-08-04 (observer-fix sub-campaign):** post-restart Smoke-1 FAIL — observer dispatch error "Model not found: github-copilot/gemini-3.6-flash" → §10 Phase 1 root-cause: cebula preset observer array still had stale/nonexistent gemini-3.6-flash as primary (F7-incomplete — only opencode-go preset updated) → Phase 4 cod-3 3-edit fix (oh-my-opencode-slim.jsonc cebula observer array: opencode-go/kimi-k2.7-code → [0] primary, gemini-3.6-flash → gemini-3.5-flash fallback; dcp.jsonc modelMaxLimits/modelMinLimits; ai-assist-sources.yaml role_mapping label) → Phase 5 ai--3 APPROVE-WITH-FINDINGS (F1 ACCEPTED → superseded note in 2026-08-03-cebula-copilot-rebalance.md, F2 deferred, F3 closed, F4 no-action) → Smoke-1 re-run (obs-2) PASS — observer resolves to opencode-go/kimi-k2.7-code, multimodal confirmed. Change set uncommitted (owner commit decision pending).
