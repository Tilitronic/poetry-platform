# DIA-163 batch-aware A1 plugin - delegation-observer warns only on unsafe parallel task batches (2026-08-12)

- **Date:** 2026-08-12
- **Source:** DIA-163 fix ticket (proposal 5 from the DIA-159 task-parallelization analysis, ai--3 session report); coder-lane implementation + this AGENTS.md section 2.5 close-out registration lane (docs lane).
...
- **Outcome:** the delegation-observer A1 check (tool.execute.before hook, ~line 733) is now batch-aware: READ_ONLY_LANES {researcher, ai-specialist, ai-auditor, code-navigator, observer}, WRITER_LANES {analyzer, conspecter, memory-manager}; isSafeTaskBatch() classifies parallel task() batches as SAFE (A read-only fan-out / B single-writer+readers -> silent) vs UNSAFE (two writers, two coders, etc. -> warning + a1_violation registry row, advisory not blocking). After DIA-162 legalized approved parallel batches, this keeps A1 from flagging every legitimate batch while preserving the warning for genuinely unsafe concurrency.

## Ticket

- **DIA-163** (Medium, VERIFIED) - "Make delegation-observer A1 warning batch-aware (only warn on unsafe parallel task batches)".
- **Related:** DIA-159 (task-parallelization analysis, parent), DIA-162 (BATCH-DISPATCH rule, sibling proposals 1-4).
...
- **Header comments** updated to batch-aware description matching the DIA-162 BATCH-DISPATCH A/B/C rule.
