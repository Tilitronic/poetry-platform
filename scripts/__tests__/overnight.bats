#!/usr/bin/env bats
# DIA-126(a) overnight.sh unit tests (2026-08-13, Option A full lane).
#
...
@test "overnight: TUI mode launches fake opencode with hardened env" {
  ...
  assert_output_contains 'OPENCODE_PERMISSION={"bash":{"rm -rf *":"deny","rm *":"deny","rmdir *":"deny","chmod *":"deny","chown *":"deny"}}'
  assert_output_contains "hardened: rm / rm -rf / rmdir / chmod / chown -> DENY"
}
