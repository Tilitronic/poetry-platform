...
# volta-to-mise §2.1) and Dockerfile.dev ARGs; asserts parity, reads only.
# Exit precedence 2>1>0: 0 match; 1 parity violated (report-ALL); 2 INFRA
# (missing file, duplicate key/ARG, missing ARG). Bash-3 compatible.
set -euo pipefail
...
# parse_dockerfile <file>: ARG NODE_VERSION=/PNPM_VERSION= -> DOCKER_NODE/DOCKER_PNPM.
# Docker ARG resolves last-wins; a duplicate/missing declaration is INFRA.
parse_dockerfile() {
  local file="$1" node_count pnpm_count
  node_count="$(grep -cE '^[[:space:]]*ARG[[:space:]]+NODE_VERSION=' "$file" || true)"
  pnpm_count="$(grep -cE '^[[:space:]]*ARG[[:space:]]+PNPM_VERSION=' "$file" || true)"
  [ "${node_count}" -le 1 ] || { echo "fail: source defective: Dockerfile.dev has duplicate ARG 'NODE_VERSION'" >&2; return 1; }
  [ "${pnpm_count}" -le 1 ] || { echo "fail: source defective: Dockerfile.dev has duplicate ARG 'PNPM_VERSION'" >&2; return 1; }
  [ "${node_count}" -eq 1 ] || { echo "fail: source defective: Dockerfile.dev is missing ARG 'NODE_VERSION'" >&2; return 1; }
  [ "${pnpm_count}" -eq 1 ] || { echo "fail: source defective: Dockerfile.dev is missing ARG 'PNPM_VERSION'" >&2; return 1; }
...
# compare <tool> <mise_val> <docker_val>: normalize both, emit ok:/fail: (report-ALL).
compare() {
  local tool="$1" mise_val="$(strip_value "$2")" docker_val="$(strip_value "$3")"
  if [ "${mise_val}" = "${docker_val}" ]; then
    echo "ok: ${tool} ${docker_val} (parity)"
    ok=$((ok + 1))
    return 0
  fi
  echo "fail: ${tool} — .mise.toml=${mise_val} Dockerfile.dev=${docker_val}" >&2
  fail=$((fail + 1))
  return 1
}
...
parse_dockerfile "${DOCKERFILE_DEV}" || infra_summary

compare node "${MISE_NODE}" "${DOCKER_NODE}" || true
compare pnpm "${MISE_PNPM}" "${DOCKER_PNPM}" || true
aggregate
