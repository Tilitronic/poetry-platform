...
> **Decision source:** architector design for DIA-132 "Parallel Coders (Batch D Expansion)" (approved 2026-08-13); DIA-135 coder prompt hygiene policies (2026-08-14)
...
delegations (Singleton-Batch Semantic Exemption), and the DIA-135 coder-lane
...
record the DIA-135 policy decisions named in the DIA-135 CHANGELOG entry. They are
...
- **Consequences:** Codified in AGENTS.md section 2.3, coder_append.md, orchestrator_append.md (rule R4), and all 3 OMO preset prompts (byte-identical INSTANCE-SEPARATION RULE block). Supersedes the DIA-134 same-session GREEN reuse practice.
- **Alternatives Considered:** Allow same-session reuse when context continuity helps (rejected: weakens the independence of RED assertions from the implementing instance); no codification (rejected: ambiguity caused the DIA-134 pattern).
